...
export async function runScanCoi(input: unknown) {
...
export async function runScanW9(input: unknown) {
    const data = ScanInput.parse(input);
    {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: sub, error } = await (supabaseAdmin.from("subcontractors" as any) as any)
...
export async function runVerifyLicense(input: unknown) {
    const data = LicenseInput.parse(input);
    {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
