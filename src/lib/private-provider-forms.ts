...
async function initDoc(): Promise<{ pdf: PDFDocument; ctx: DrawCtx }> {
  ...
  pdf.setCreator("Cleard");
  pdf.setProducer("Cleard");
...
export async function generateNTBO(fields: NTBOFields): Promise<Uint8Array> {
  ...
  drawFooter(ctx, "Generated by Cleard — review before submission.");
  return pdf.save();
}
...
export async function generateNOC(fields: NOCFields): Promise<Uint8Array> {
  const { pdf, ctx } = await initDoc();
  drawHeader(
    ctx,
  ...
  drawWrapped(
    ctx,
    "State of Florida. The undersigned hereby gives Notice that improvement will be made to certain real property, and in accordance with Chapter 713, Florida Statutes, the following information is provided in this Notice of Commencement.",
    { size: 9.5, color: MUTED, lineGap: 3 },
  );
  ctx.y -= 6;

  drawSectionTitle(ctx, "1. Property");
  drawLabelValue(ctx, "Property Address", fields.propertyAddress);
  drawLabelValue(ctx, "Parcel / Tax ID (PCN)", fields.parcelTaxId);
  drawLabelValue(ctx, "Legal Description", fields.legalDescription);

  drawSectionTitle(ctx, "2. Description of Improvement");
  drawLabelValue(ctx, "General Description", fields.improvementDescription);

  drawSectionTitle(ctx, "3. Owner Information");
  drawLabelValue(ctx, "Owner Name / Interest", fields.ownerName);
  drawLabelValue(ctx, "Owner Address", fields.ownerAddress);

  drawSectionTitle(ctx, "4. Contractor");
  drawLabelValue(ctx, "Contractor Name", fields.contractorName);
  drawLabelValue(ctx, "Contractor Address", fields.contractorAddress);
  drawLabelValue(ctx, "Contractor License #", fields.contractorLicense);
  drawLabelValue(ctx, "Contractor Phone", fields.contractorPhone);

  drawSectionTitle(ctx, "5. Surety (if any)");
  drawLabelValue(ctx, "Surety Name", fields.suretyName || "N/A");
  drawLabelValue(ctx, "Surety Address", fields.suretyAddress || "N/A");
  drawLabelValue(ctx, "Bond Amount", fields.suretyBondAmount || "N/A");

  drawSectionTitle(ctx, "6. Lender (if any)");
  drawLabelValue(ctx, "Lender Name", fields.lenderName || "N/A");
  drawLabelValue(ctx, "Lender Address", fields.lenderAddress || "N/A");

  // Page 2 for statutory language + signature/notary block.
  const page2 = pdf.addPage([PAGE_W, PAGE_H]);
  ctx.page = page2;
  ctx.y = PAGE_H - 60;

  drawSectionTitle(ctx, "7. Persons Designated to Receive Notices (§713.13(1)(b))");
  drawLabelValue(ctx, "Designee", fields.designProfessional || "See owner above");
  drawLabelValue(ctx, "Designee Address", fields.designProfessionalAddress || "");

  drawSectionTitle(ctx, "8. Expiration");
  drawWrapped(
    ctx,
    "Expiration date of this Notice of Commencement is one (1) year from the date of recording, unless a different date is specified. WARNING TO OWNER: ANY PAYMENTS MADE BY THE OWNER AFTER THE EXPIRATION OF THIS NOTICE OF COMMENCEMENT ARE CONSIDERED IMPROPER PAYMENTS UNDER CHAPTER 713, PART I, SECTION 713.13, FLORIDA STATUTES, AND CAN RESULT IN YOUR PAYING TWICE FOR IMPROVEMENTS TO YOUR PROPERTY. A NOTICE OF COMMENCEMENT MUST BE RECORDED AND POSTED ON THE JOB SITE BEFORE THE FIRST INSPECTION. IF YOU INTEND TO OBTAIN FINANCING, CONSULT WITH YOUR LENDER OR AN ATTORNEY BEFORE COMMENCING WORK OR RECORDING YOUR NOTICE OF COMMENCEMENT.",
    { size: 8.5, color: INK, lineGap: 3 },
  );

  ctx.y -= 20;
  drawSectionTitle(ctx, "Signature");
  ctx.page.drawLine({
    start: { x: MARGIN, y: ctx.y },
    end: { x: MARGIN + 260, y: ctx.y },
    thickness: 0.8,
  ...
  ctx.y -= 12;
  ctx.page.drawText("Signature of Owner or Owner's Authorized Officer", { x: MARGIN, y: ctx.y, size: 8, font: ctx.font, color: MUTED });
  ctx.page.drawText("Date", { x: PAGE_W - MARGIN - 140, y: ctx.y, size: 8, font: ctx.font, color: MUTED });

  ctx.y -= 30;
  ctx.page.drawText("Print Name:  ______________________________________________", {
    x: MARGIN, y: ctx.y, size: 10, font: ctx.font, color: INK,
  });
  ctx.y -= 24;
  ctx.page.drawText("Title / Capacity:  ______________________________________________", {
    x: MARGIN, y: ctx.y, size: 10, font: ctx.font, color: INK,
  });

  ctx.y -= 30;
  drawSectionTitle(ctx, "Notary Acknowledgment");
  drawWrapped(
    ctx,
    "State of Florida, County of ____________________. The foregoing instrument was acknowledged before me by means of ☐ physical presence or ☐ online notarization, this ______ day of ______________, 20____, by ______________________________, who is personally known to me or has produced ______________________________ as identification.",
    { size: 9, color: INK, lineGap: 3 },
  );
  ctx.y -= 20;
  ctx.page.drawLine({ start: { x: MARGIN, y: ctx.y }, end: { x: MARGIN + 260, y: ctx.y }, thickness: 0.8, color: INK });
  ctx.y -= 12;
  ctx.page.drawText("Notary Public Signature & Seal", { x: MARGIN, y: ctx.y, size: 8, font: ctx.font, color: MUTED });

  drawFooter(ctx, "Generated by Cleard — record with County Clerk before first inspection.");
  return pdf.save();
}

// -------- generic agreement (PAA, LPOA, payment authorization, trade authorization) --------
...
export type AgreementFields = {
  title: string;
  subtitle: string;
  /** Short paragraph under the header, before any fields. */
  intro?: string;
  /** Label/value pairs printed as a facts block above the clauses. */
  facts?: Array<{ label: string; value: string }>;
  sections: AgreementSection[];
  /** Caption under the signature rule, e.g. "Contractor Signature". */
  signatureLabel?: string;
  footer?: string;
};

function estimateLines(text: string, font: PDFFont, size: number): number {
...
/**
 * Multi-page contract PDF. The clause text is the same content the on-screen
 * agreement renders, so what a signer reads in the portal is what SignWell sends.
 */
export async function generateAgreementPdf(input: AgreementFields): Promise<Uint8Array> {
  const { pdf, ctx } = await initDoc();
  drawHeader(ctx, input.title, input.subtitle);

  const breakIfNeeded = (needed: number) => {
    if (ctx.y - needed >= 80) return;
    ctx.page = pdf.addPage([PAGE_W, PAGE_H]);
    ctx.y = PAGE_H - 72;
  };

  if (input.intro) {
    drawWrapped(ctx, input.intro, { size: 9.5, color: MUTED, lineGap: 3 });
    ctx.y -= 6;
  }

  for (const f of input.facts ?? []) {
    breakIfNeeded(46);
    drawLabelValue(ctx, f.label, f.value);
  }

  for (const s of input.sections) {
    const bodyHeight = estimateLines(s.body, ctx.font, 9.5) * 12.5;
    breakIfNeeded(bodyHeight + (s.heading ? 30 : 0));
    if (s.heading) drawSectionTitle(ctx, s.heading);
    drawWrapped(ctx, s.body, { size: 9.5, lineGap: 3 });
    ctx.y -= 8;
  }
  ...
  ctx.y -= 12;
  ctx.page.drawText(pdfSafe(input.signatureLabel ?? "Signature"), {
    x: MARGIN,
    y: ctx.y,
    size: 8,
    font: ctx.font,
  ...
  drawFooter(ctx, input.footer ?? "Generated by Cleard.");
  return pdf.save();
}
