...
import { Scale, Plus, Trophy, Award, ChevronRight, X } from "lucide-react";
    ...
    <div className="fixed inset-0 z-50 bg-obsidian/50 flex items-start justify-center overflow-y-auto p-4">
      <div className="w-full max-w-2xl bg-white rounded-[3px] shadow-2xl my-16">
        <div className="flex items-center justify-between px-6 py-4 border-b border-obsidian/10">
          <div>
            <div className="eyebrow text-obsidian/50">New Comparison</div>
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-[3px] hover:bg-obsidian/5"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((p) => (
              ...
              >
                ...
                {p.permit_number && (
                  <div className="font-mono text-[10px] text-obsidian/50 shrink-0">
                    {p.permit_number}
                  </div>
                )}
