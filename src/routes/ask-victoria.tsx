...
export const Route = createFileRoute("/ask-victoria")({
  head: () => ({
    meta: [
      { title: "Ask Victoria — Cleared by Flōridian" },
...
function mockReply(q: string): string {
  ...
  if (t.includes("553.791") || t.includes("private provider"))
    return "Under **FS §553.791**, a licensed private provider may perform plan review and inspections in lieu of the local building official. The local government must reduce the building permit fee accordingly (§553.791(2)(b)), and the statutory clock runs **10 business days** for permit-or-cite once your affidavit of intent is filed.";
  if (t.includes("hb 803"))
    return "**HB 803** (effective Jul 1, 2026) requires Florida counties to reduce permit fees when a private provider is used: **25% for partial** (plan review *or* inspections) and **50% for full** (both). This stacks on top of any existing §553.791(2)(b) reduction the county already applies.";
  if (t.includes("inspection"))
    return "Cleared performs **real-time virtual inspections** with a **48-hour correction window**. If a correction is needed, the inspector posts findings to the project record and the trade has 48 business hours to remediate before a re-inspection is required.";
  ...
  if (t.includes("palm beach") || t.includes("martin") || t.includes("indian river") || t.includes("st lucie") || t.includes("st. lucie"))
    return "That jurisdiction is inside Cleared's service area (**Palm Beach + Treasure Coast**). I can pull the current submittal checklist, ePlan portal status, and fee schedule from the Building Dept Logins vault — want me to summarize it for a specific permit type?";
  if (t.includes("fee") || t.includes("cost"))
    return "Cleared invoices two line items at submittal: **Permitting Fee = construction value × 1.5%** and a flat **Private Provider & Admin Fee of $8,856**. Both appear on your Invoices tab and are auto-disbursed to the county on permit issuance.";
  return "Good question. For permitting matters I draw on **FS §553.791**, the **Florida Building Code 8th Ed. (2023)**, and county-specific requirements across Palm Beach and the Treasure Coast. Could you narrow it to a specific permit type, address, or statute section?";
}
