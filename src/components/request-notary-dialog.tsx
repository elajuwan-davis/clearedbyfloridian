...
export function RequestNotaryDialog({
  ...
  /** Live permit UUID required for DB insert. */
  permitId: string;
  projectName?: string;
  documentName: string;
  docId?: string;
  ...
  async function submit() {
    ...
    try {
      ...
      toast.success("Notary request submitted");
      onSubmitted?.();
      onOpenChange(false);
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Failed to submit notary request");
    } finally {
      setBusy(false);
    }
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md rounded-[3px]">
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          <Button variant="dark" className="rounded-[3px]" onClick={() => void submit()} disabled={busy}>
            {busy ? (
