...
-- Hard invariant: 'signed' is only reachable together with provider_confirmed, so no path
-- (client, RPC, or staff attestation) can declare a signature complete without SignWell.
ALTER TABLE public.signature_requests
  DROP CONSTRAINT IF EXISTS signature_requests_signed_requires_provider_check;
ALTER TABLE public.signature_requests
  ADD CONSTRAINT signature_requests_signed_requires_provider_check
    CHECK (status <> 'signed' OR status_source = 'provider_confirmed');
...
-- NOTE: no grandfathering. Rows created under the old browser-only flow are deliberately
-- left as 'draft'/'staff_attested' — only a verified SignWell webhook may mark a signature
-- signed, so legacy rows are treated as unsigned.
...
ALTER TABLE public.paa_signatures
  DROP CONSTRAINT IF EXISTS paa_signatures_signed_requires_provider_check;
ALTER TABLE public.paa_signatures
  ADD CONSTRAINT paa_signatures_signed_requires_provider_check
    CHECK (status <> 'signed' OR status_source = 'provider_confirmed');
...
CREATE TABLE IF NOT EXISTS public.payment_authorizations (
  ...
  CONSTRAINT payment_authorizations_signed_requires_provider_check
    CHECK (status <> 'signed' OR status_source = 'provider_confirmed')
);
...
CREATE TABLE IF NOT EXISTS public.lpoa_signatures (
  ...
  revoked_at timestamptz,
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT lpoa_signatures_signed_requires_provider_check
    CHECK (status <> 'signed' OR status_source = 'provider_confirmed')
);
