...
COMMENT ON COLUMN public.tenants.plan IS
  'Feature tier: ''full'' (managed/invited account, everything on; the default) or ''trial'' (self-serve /join signup: own permits, own portal logins, messages to Cleard). Switchable per tenant from /admin/plans. See src/lib/plan-access.ts.';
